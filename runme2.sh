...
rm /etc/dhcpd.conf # if exist, erase it
...
echo "  option domain-name \"$network_name\";" >> /etc/dhcpd.conf
echo "  option domain-name-servers 192.168.2.1;" >> /etc/dhcpd.conf
echo "  range 192.168.2.2 192.168.2.40;" >> /etc/dhcpd.conf
echo "}" >> /etc/dhcpd.conf
progress_bar
sleep_mode
...
gnome-terminal -e "airbase-ng -e $network_name -c $_channel "$in_interface"mon"
sleep_mode
